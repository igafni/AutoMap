...
def guess_dumpfile_type_and_os(dumpfile_string_data):
    """Returns file_type, os when any line of the input file matches the regexp"""
    for line in dumpfile_string_data:
        logger.debug("line:\n{}".format(line))
...
def parse_linux_tr(dumpfile_string_data, ip):
    hops = []
    if ip is None:
        MyException("Linux ARP does not contain the IP of the "
...
def parse_linux_arp(dumpfile_string_data):
    nodes = []
    for line in dumpfile_string_data:
...
def parse_windows_arp(dumpfile_string_data, ip):
...
def parse_linux_route(dumpfile_string_data):
